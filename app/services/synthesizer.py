...
class Synthesizer:
    ...
    def dataframe_to_json(
        ...
        # Normalize column names for content
        if 'contents' in context.columns and 'content' not in context.columns:
            context_copy['content'] = context_copy['contents']
        
        # Handle FAQ data - if we have question/answer columns, create content
        if 'question' in context.columns and 'answer' in context.columns:
            context_copy['content'] = context_copy.apply(
        ...
        for col in columns_to_keep:
            if col in context_copy.columns:
                available_columns.append(col)
        
        # If no specified columns are available, use content/contents or all columns
        if not available_columns:
            if 'content' in context_copy.columns:
                available_columns = ['content']
            elif 'contents' in context_copy.columns:
                available_columns = ['contents']
            else:
                # Exclude embedding column (too large)
                available_columns = [col for col in context_copy.columns if col != 'embedding']
        
        return context_copy[available_columns].to_json(orient="records", indent=2)
